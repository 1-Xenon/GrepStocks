...
if (isset($_POST['edit-review-submit'])) {
    // Validate CSRF token
    ...
    if (!$username) {
        ...
        exit('Error: You must be logged in to edit a review.');
    }

    $reviewRating = intval($_POST['edit-review-rating']); // Ensure integer value
    // Check if the rating is within the expected range
    if ($reviewRating < 1 || $reviewRating > 5) {
        exit('Error: Invalid rating value.');
    }

    $review = sanitizeInput($_POST['edit-review-comment']);
    ...
    // Prepare and execute UPDATE statement securely
    $sql = "UPDATE reviews SET productID = ?, userID = ?, reviewRating = ?, review = ?, reviewEditTime = NOW() WHERE reviewID = ?";
    if ($stmt = mysqli_prepare($con, $sql)) {
        mysqli_stmt_bind_param($stmt, "iiisi", $productID, $userID, $reviewRating, $review, $reviewID);
